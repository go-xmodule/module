...
func (t *Grid) SetSelectWhere(where models.WhereClosure) *Grid {
	t.where = where
	return t
}

// GetSelectWhere 获取当前数据查询条件
func (t *Grid) GetSelectWhere() models.WhereClosure {
	return t.where
}
...
func (t *Grid) GetTable(pageData utils.PageData) *Grid {
	t.PageData = pageData
	var response [][]Field
		...
		for _, v := range res {
			...
			for _, key := range t.fieldSort {
				if key == "created_at" || key == "updated_at" || key == "deleted_at" {
					if v[key] != nil {
						ts, _ := time.ParseInLocation(utils.ParseTimeTemplate, v[key].(string), time.Local)
						value = ts.Format(utils.DateTemplate)
					}
				} else {
					value = v[key]
				}
