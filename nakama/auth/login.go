...
func (stc *ConsoleTokenClaims) Valid() error {
	...
	if stc.ExpiresAt <= time.Now().UTC().Unix() {
		...
		utils.Logger.Warning("Token is expired")
...
func (a *Auth) testToken(loginToken LoginToken) (int, error) {
	...
	utils.Logger.Debug("parse_console_token:", " uname:", uname, "  email:", email, "  role:", role, "  exp:", exp, "  ok:", ok)
	if !ok || !token.Valid {
		// The token or its claims are invalid.
		utils.Logger.Error("console login  token or its claims are invalid")
		return InvalidToken, err
	}
	if exp <= time.Now().UTC().Unix() {
		// Token expired.
		utils.Logger.Error("console login  token login expired")
...
func (a *Auth) login() (LoginToken, error) {
	...
	if !utils.Success(response.StatusCode()) {
		utils.Logger.Error("request nakama server error", response)
	...
	// utils.Logger.Debug("parseConsoleToken:", " uname:", uname, "  email:", email, "  role:", role, "  exp:", exp, "  ok:", ok)
